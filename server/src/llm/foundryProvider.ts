...
export class FoundryProvider implements LlmProvider {
  ...
  private readonly endpoint = process.env.AZURE_AI_FOUNDRY_ENDPOINT?.replace(/\/$/, "") ?? "";
  private readonly deployment = process.env.AZURE_AI_FOUNDRY_DEPLOYMENT ?? "";
  private readonly apiVersion = process.env.AZURE_AI_FOUNDRY_API_VERSION ?? "2024-08-01-preview";
  ...
  private async complete(messages: ChatMessage[], temperature: number): Promise<string> {
   ...
   */
  async extractMetrics(rawText: string, candidates: MetricPair[]): Promise<MetricPair[]> {
    if (candidates.length === 0) return [];
    const system: ChatMessage = {
      role: "system",
      content:
        "You normalise product specification text. You are given RAW_TEXT and CANDIDATES " +
