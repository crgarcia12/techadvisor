...
const SYSTEM_PROMPT: ChatMessage = {
  role: "system",
  content:
    "You are TechAdvisor, an assistant that helps a user decide which technology product to buy " +
...
function ruleBasedReply(history: ChatMessage[]): string {
  ...
  const mentions = (words: string[]) => words.some((w) => lastUser.includes(w));
...
const CONFIG_MISSING_NOTE =
  "⚠️ Azure AI Foundry is not configured, so I'm using a built-in guided flow. " +
...
export async function runChat(history: ChatMessage[]): Promise<ChatResult> {
  ...
  try {
    ...
    return {
      reply: `${ruleBasedReply(history)}\n\n(Note: the AI provider returned an error: ${(err as Error).message})`,
      configMissing: false,
    };
  }
}
